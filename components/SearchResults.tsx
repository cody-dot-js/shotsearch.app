      ...
      <header className="shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <h2 className="m-0 mr-4 mb-0 text-2xl font-bold leading-tight text-gray-900">
            {source} {!isLoading && images && <>({images.length})</>}
          </h2>
        </div>
      </header>
      <Loading isLoading={isLoading} className="h-48 p-6" />
      <EmptyResults images={images} searchValue={searchQuery} source={source} />
      <ImagePreviews images={images || undefined} />
    </div>
  </article>
);
